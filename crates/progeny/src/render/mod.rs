...
// `client`, `operations`, `server` and `types` are crate-visible rather than private because the
...
pub(crate) fn run(
    ...
    match config.packaging {
        Packaging::Crate => {
            ...
            files.insert(Utf8PathBuf::from("src/lib.rs"), crate_root(&modules));
...
/// The one-crate root: a declaration per module, the two edges behind their features and the
/// reflection beside the types, ungated.
fn crate_root(modules: &[(&str, TokenStream)]) -> String {
    let declarations = modules.iter().map(|(name, _)| {
        let ident = quote::format_ident!("{name}");
        // The support module is not part of the generated crate's public API; it is reachable
        // because the type modules call into it and for no other reason.
        let hidden = (*name == "support").then(|| quote! { #[doc(hidden)] });
...
#[expect(
    ...
    reason = "a workspace is the product of the model, the configuration and one rendered body \
              per crate; threading them through a struct would put the same values one \
...
fn workspace_files(
    ...
    // resolves in the server module exactly as it does in the other two packagings.
    let reflection = operations_body.map(|_| {
        quote! {
            pub use #types_crate::operations;
        }
    });
...
fn insert_types_crate(
    ...
    let operations = operations_body.map(|_| {
        quote! {
            pub mod operations;
        }
    });
    ...
    if let Some(operations) = operations_body {
        files.insert(directory.join("src/operations.rs"), source(operations));
    }
    if needed {
        files.insert(
...
mod tests {
    ...
    /// One operation returning one component: the smallest document with all three crates.
    ...
    fn a_workspace_has_three_feature_isolated_crates() {
        ...
        let rendered = files(pets_document(), &config)?;
    ...
    /// The reflection lives in the types crate and both edges re-export it beside `types`, so
    /// `super::operations` resolves in the server module exactly as it does in the one-crate
    /// packaging.
    #[test_util::test]
    fn a_workspace_re_exports_the_reflection_beside_the_types() {
        let config = Config {
            packaging: Packaging::Workspace,
            ..Config::default()
        };
        ...
        for half in ["client", "server"] {
            ...
            let root = rendered
                .get(camino::Utf8Path::new(&path))
                .ok_or_eyre(path)?;
            assert!(root.contains("pub use api_types::operations;"), "{root}");
        }
        let types_root = rendered
            .get(camino::Utf8Path::new("api-types/src/lib.rs"))
            .ok_or_eyre("types root")?;
        assert!(types_root.contains("pub mod operations;"), "{types_root}");
        let operations = rendered
            .get(camino::Utf8Path::new("api-types/src/operations.rs"))
            .ok_or_eyre("the reflection lives in the types crate")?;
        assert!(operations.contains("pub enum Route"), "{operations}");
        let server = rendered
            .get(camino::Utf8Path::new("api-server/src/server.rs"))
            .ok_or_eyre("server module")?;
        assert!(
            server.contains("operations::Route::ListPets.path()"),
            "{server}"
        );
    }

    /// The reflection is emitted wherever there is something to call, in every packaging, and
    /// the router registers its templates through it rather than beside it.
    #[test_util::test]
    fn the_reflection_rides_beside_the_types_in_every_packaging() {
        let document = json!({
            "openapi": "3.1.0",
            "paths": {
                "/pets": {
        ...
        let as_crate = files(document.clone(), &Config::default())?;
        let lib = &as_crate[camino::Utf8Path::new("src/lib.rs")];
        // Ungated: the module has no dependency, so neither feature has anything to guard.
        assert!(lib.contains("pub mod types;\npub mod operations;"), "{lib}");
        let operations = &as_crate[camino::Utf8Path::new("src/operations.rs")];
        syn::parse_file(operations)?;
        assert!(
            operations.contains("ListPets,\n    CreatePets,"),
            "{operations}"
        );
        let server = &as_crate[camino::Utf8Path::new("src/server.rs")];
        ...
        let nested = &files(document.clone(), &module)?[camino::Utf8Path::new("progeny.rs")];
        assert!(nested.contains("pub mod operations {"), "{nested}");
        assert!(nested.contains("pub enum Operation {"), "{nested}");

        // Turning the edges off does not turn the reflection off: it is what a program that
        // brings its own HTTP stack keys on.
        let types_only = Config {
            emit: crate::config::Emit {
                types: true,
                client: false,
        ...
        let rendered = files(document, &types_only)?;
        let names: Vec<&str> = rendered.keys().map(|path| path.as_str()).collect();
        assert_eq!(
            names,
            [
                "Cargo.toml",
    ...
    /// The stem guard, at the one place it used to panic: a digit-led `operationId` is a
    /// valid identifier throughout, and the same spelling in all three surfaces.
    #[test_util::test]
    fn a_digit_led_operation_id_renders_in_every_surface() {
        let document = json!({
            "openapi": "3.1.0",
            "paths": {
                "/2fa/verify": {
                    "post": {"operationId": "2fa_verify", "responses": {"204": {"description": "ok"}}},
                },
            },
        });
        let rendered = files(document, &Config::default())?;
        ...
        assert!(
            client.contains("pub fn _2fa_verify(&self) -> _2faVerifyRequest<'_>"),
            "{client}"
        );
        let server = &rendered[camino::Utf8Path::new("src/server.rs")];
        assert!(server.contains("pub enum _2faVerifyResponse"), "{server}");
        assert!(
            server.contains("operations::Route::_2faVerify.path()"),
            "{server}"
        );
        let operations = &rendered[camino::Utf8Path::new("src/operations.rs")];
        assert!(
            operations.contains("rust_name: \"_2fa_verify\""),
            "{operations}"
        );
    }
