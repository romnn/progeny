...
impl StyleContract {
    ...
    /// Whether the schema says this parameter is an array — the fact the reading side cannot
    /// recover from the wire, because `ids=a,b` under non-exploded `form` is byte-identical to a
    /// scalar containing a comma.
    pub(crate) fn array(self) -> bool {
        matches!(self.shape, ParamShape::Array)
    }
    ...
    pub(crate) fn erased_by_explosion(self) -> bool {
        matches!(
            ...
            (Style::Form, true, ParamShape::Object)
        )
    }
